...
This directory separates product, technical, security, and verification concerns.
...
Open `index.html` to browse these documents in a browser. Double-click `serve.cmd` first; browsers block a page from reading sibling files over `file://`.
...
- Add the document's entry to `index.html` in the same change.
